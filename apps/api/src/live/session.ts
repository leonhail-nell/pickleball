...
export class LiveSession {
  ...
  static async start(sessionId: string, io: Server): Promise<LiveSession> {
    ...
    const openGames = await prisma.game.findMany({
      ...
      include: { players: { include: { user: { select: { name: true, rating: true } } } }, court: true },
    });
    for (const g of openGames) {
      const team = (t: string) =>
        g.players.filter((p: GPU) => p.team === t).map((p: GPU) => ({
          id: p.userId, name: p.user.name, rating: p.user.rating,
        }));
      if (g.status === 'LIVE') {
  ...
  async checkIn(user: { id: string; name: string; rating: number }, forceProrated: boolean) {
  ...
  /** Host edit: propagate a player's new name/rating into the live session. */
  async updatePlayer(userId: string, name?: string, rating?: number): Promise<void> {
    ...
    const patch = (team: NamedPlayer[]) =>
      team.map((p) => (p.id === userId
        ? { ...p, name: name ?? p.name, rating: rating ?? p.rating }
        : p));
  ...
  /** Host: detach a court from this session. If it has a live game, pass
   *  `force` to void that game first, then remove the court. */
  async removeCourt(courtId: string, actorId: string, force = false): Promise<void> {
    const court = this.courts.get(courtId);
    if (!court) throw new Error('court is not attached to this session');
    const hadGame = !!court.gameId;

    if (court.gameId) {
      if (!force) throw new Error('finish or move the game on this court first');
      // Void the in-progress game *inline* (court stays in the map so
      // releaseCourt can find it), then detach the court, then refill once —
      // this avoids finishGame's internal fillCourts re-assigning onto a court
      // we're about to delete.
      const gameId = court.gameId;
      const { teamA, teamB } = await this.releaseCourt(gameId);
    ...
    await this.audit('court_removed', actorId, { courtId, number: court.number, forced: force });

    if (hadGame) {
      // players from the voided game go back into rotation on remaining courts
      await this.fillCourts(); // broadcasts
    } else {
      this.broadcast();
    }
  ...
  private async releaseCourt(gameId: string) {
    ...
    // A doubles game always has exactly two players per team, so these are tuples.
    const teamA = game.players
      .filter((p: GP) => p.team === 'A')
      .map((p: GP) => p.userId) as [string, string];
    const teamB = game.players
      .filter((p: GP) => p.team === 'B')
      .map((p: GP) => p.userId) as [string, string];
  ...
  private named(id: string): NamedPlayer {
    ...
    return { id, name: p?.name ?? this.names.get(id) ?? id, rating: p?.rating ?? 3 };
  }
